...
* 数组实现的队列,会有数据搬移的操作
* 数组实现队列,在数据出列的时候, 会删除小标为0的元素, 然后导致数据搬移
...
* 取中间的逆序对个数为n*(n-1)/4,则需要这么多次比较,平均复杂度为O\\((n^2)\\)
	...
	void bubbleSort(int a[],int n){
		...
		for (int i = 0; i < n; ++i){
			...
			// 每次找到最大的数,
			// n - i表示当前未被排序的元素个数
			// n - i - 1表示未排序部分最右边的索引,也是存放本轮排序最大的数,注意j < 是小于号,a[j+1]不会越界
...
* 插入排序的核心思想: 
	* 将数据分为 已排序区间 和 未排序区间
	* 初始已排序区间只有一个元素
	* 从未排序区间取出元素, 从后到前找到合适的位置插入,并保证已排序区间一直有序
	* 重复这个过程
...
* 如果输入序列是有序的,或者是不分有序的,则退化成O\\((n^2)\\),因为每次划分只能使待排序序列减一.
* 快排优化: 
	* 随机选取基准
	* 在待排序中,选3个数,取中间的数
	* 如果待排序列的个数小于N(N可以取10)的时候,可以将采用插入排序.
	* 与基准数比较结束后,把与key相等的元素移到枢轴范围
...
* 为什么时间复杂度是O(n)?
	...
	当m->1时,n*log(n/m) -> nlogn等于快排
	
	平均来看log(n/m) -> C , 即平均复杂度是O(Cn) -> O(n)
	```
	
	* 如果是m->1最坏的情况, 那么就跟快排没区别, 这也是桶排序用不多的原因
...
* 10万个手机号排序,可以低位到高位进行11次桶排序, 为什么从低到高?
	* 权重高的后排序,才能保证权重低的排序有效
	* 要保证每次排序的稳定性
* 牛津字典单词排序
...
* 为链表加多级索引的结构就是跳表.多级,比如原始元素为n个,每2个去一个索引,则一级索引就有n/2个索引,二级就有4/n个索引,...一直到最上层的索引为2个
* 查找速度为O(logn)
...
* 散列冲突解决	
	* 开放寻址法
		...
		* 装载因子 = 填入表中的个数/散列表的长度
		* 删除数据时,需要标记该位置是删除状态,而不是空的状态,为什么?当采用线性探测去解决冲突时,可能会遇到被删除的index,如果这个index被占用,则hash算法[]()不准了
	* 链表法
...
* 负载均衡,将用户的请求尽可能均匀分配到不同的缓存服务器
	...
	* 如何实现一个粘滞的负载均衡算法,同一个会话中的所有请求路由到同一个服务器上?
		...
		* 可以借助哈希算法,将会话id或者ip的哈希值,对服务器列表取余,得到的就是对应的服务器编号
...
* 二叉搜索树那么强大为啥还有红黑树?
	* 因为插入数据的时候,数据可能集中在一起, 使二叉树高度很高,搜索效率降低
* 平衡二叉树: 严格的定义是这样的,二叉树中任意一个结点的左右字数的高度相差不能大于1.完全二叉树,满二叉树都是平衡二叉树
* AVL(最先发明的自平衡二叉树)数满足严格的平衡二叉树
...
* 堆的插入过程,如果将一个数据插入到末尾可能会违反堆的特性.需要堆化
...
* 堆的弹出数据.将堆顶元素弹出.在数组上的操作是先交换堆顶元素和最后一个元素的位置.然后再堆化.(不这样的话,直接删除堆顶元素,子节点补位的话, 会造成生成的树不是完全二叉树). 如下图
...
* 表示关注和粉丝可用一个邻接表和一个逆邻接表,入度表示粉丝,出度表示关注
* 邻接表是出度表(关注), 逆邻接表为入度表(粉丝)
* 海量数据需多个机器存储.
...
* 实际地图应用用的是`A*`的启发是搜索,基于dijkstra的改进,用到了启发函数防止跑偏.但不一定是最优解,是次优解
	...
	class Graph4{
	    ...
	    // 从顶点s到顶点t的最短路径 (邻接表写法)
	    void dijkstra(int s,int t,int &w){
	        int *predeccessor = new int[v]{}; // 用来还原路径,上一个
	        ...
	        while (!queue.empty()) {
	            ...
	            list<Edge *>::iterator iter = adj[minVertext->id].begin();
	            while (iter != adj[minVertext->id].end()) {
	                Edge *e = *iter;
	                Vertex *nextVertex = vertexes[e->tid]; //minVertex->nextVertex
	                if (minVertext->dist+e->w < nextVertex->dist) {
	                    nextVertex->dist = minVertext->dist+e->w;
	                    predeccessor[nextVertex->id] = minVertext->id;
	        ...
	        // 打印输出
	        print(s, t, predeccessor);
	        cout << endl;
	    }
	    
	    // 从顶点s到顶点t的最短路径 (邻接矩阵写法)
